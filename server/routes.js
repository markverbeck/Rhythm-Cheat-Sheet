...
// get all players
router.get(
  "/hjkghkhjg86tvg/vhjuyrer678jyt/cfgh567kj",
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    if (req.body.name && req.body.password && req.body.fav && req.body.answer) {
      ...
      res.status(400).json({
        message: "Please add routerropriate information to create a new player."
      });
    }
  })
);
...
router.put(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (req.params.id && req.body.note && req.body.score) {
      const score = await functions.updateScore(
        req.params.id,
        req.body.note,
        req.body.score
      );
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const player = players.players.find(
      player => player.name == req.params.username
    );
    if (player) {
      res.json(player);
    } else {
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (player) {
      const updatePassword = await functions.changePassword(
        req.body.id,
        req.body.password
      );
      res.status(204).end();
