  ...
  <div style="perspective: 700px;">
    <div class="gameCard" :style="flipStyle">
      ...
      <div class="imgDiv" :style="imgDivStyle">
        <img :src="image" />
        <button v-on:click.prevent="startStop" class="greenButton">Get Clicking</button>
      </div>
      ...
      <GlobalEvents @keydown.shift="keyDownFunct"></GlobalEvents>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    startTimer: function() {
      ...
      this.beatInt = setInterval(() => {
        self.playSound(this.click);
        self.measureClass = "measure";
        self.beatClass = "yellowCircle yellowAnimation";
        ...
        if (this.measure === 9) {
          ...
          this.endPercentage = Math.trunc(
            (this.hit / (this.hit + this.miss)) * 100
          );
          ...
          if (this.endPercentage > this.Player.scores[note].score) {
            ...
            const newScore = axios
              .put("/api/player/" + this.Player.id, scoreSubmit)
